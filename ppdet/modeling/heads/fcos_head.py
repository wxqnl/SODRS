...
__all__ = ['FCOSFeat', 'FCOSHead', 'FCOSHead_SODRS']
...
@register
class FCOSHead_SODRS(FCOSHead):
    """
    FCOSHead of SSODRS for semi-det(ssod)
    Args:
        fcos_feat (object): Instance of 'FCOSFeat'
        num_classes (int): Number of classes
        fpn_stride (list): The stride of each FPN Layer
    ...
    def __init__(self,
                 ...
                 nms='MultiClassNMS',
                 trt=False):
        super(FCOSHead_SODRS, self).__init__()
        self.fcos_feat = fcos_feat
        self.num_classes = num_classes
        self.fpn_stride = fpn_stride
        self.prior_prob = prior_prob
        ...
        self.sqrt_score = sqrt_score
        # 定义SE模块
        self.se_module_cls = SELayer(256)
        self.se_module_reg = SELayer(256)
        #jce
        conv_cls_name = "fcos_head_cls"
        bias_init_value = -math.log((1 - self.prior_prob) / self.prior_prob)
        self.fcos_head_cls = self.add_sublayer(
    ...
    def forward(self, fpn_feats, targets=None):
        ...
        for scale_reg, fpn_stride, fpn_feat in zip(self.scales_regs,
            ...
            fcos_cls_feat, fcos_reg_feat = self.fcos_feat(fpn_feat)
            # 检查特征图的形状
            fcos_cls_feat = self.se_module_cls(fcos_cls_feat)
            fcos_reg_feat = self.se_module_reg(fcos_reg_feat)
